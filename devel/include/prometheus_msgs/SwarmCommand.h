...
struct MD5Sum< ::prometheus_msgs::SwarmCommand_<ContainerAllocator> >
{
  ...
  static const uint64_t static_value1 = 0x15ee071c075d4bbeULL;
  static const uint64_t static_value2 = 0xf4208b6edf6e01f1ULL;
};
...
struct Printer< ::prometheus_msgs::SwarmCommand_<ContainerAllocator> >
{
  template<typename Stream> static void stream(Stream& s, const std::string& indent, const ::prometheus_msgs::SwarmCommand_<ContainerAllocator>& v)
  {
    ...
    s << indent << "leader_pos[]" << std::endl;
    for (size_t i = 0; i < v.leader_pos.size(); ++i)
    {
      s << indent << "  leader_pos[" << i << "]: ";
      Printer<float>::stream(s, indent + "  ", v.leader_pos[i]);
    }
    s << indent << "leader_vel[]" << std::endl;
    for (size_t i = 0; i < v.leader_vel.size(); ++i)
    {
      s << indent << "  leader_vel[" << i << "]: ";
      Printer<float>::stream(s, indent + "  ", v.leader_vel[i]);
    }
    s << indent << "swarm_size: ";
    ...
    s << indent << "attack_target_pos[]" << std::endl;
    for (size_t i = 0; i < v.attack_target_pos.size(); ++i)
    {
      s << indent << "  attack_target_pos[" << i << "]: ";
      Printer<float>::stream(s, indent + "  ", v.attack_target_pos[i]);
    }
